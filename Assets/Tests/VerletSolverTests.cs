...
internal class VerletSolverTests
{
    ...
    public void SetUp()
    {
        ...
        m_Rope = CreateRopeWithTwoParticle(
            topParticlePosition: TopParticlePosition,
            bottomParticlePosition: BottomParticlePosition,
            cursorAttachmentParticle: AttachmentParticle.Top);
    }
    ...
    public void Test_AttachementContstraint_WithoutMovement()
    {
        ...
        m_TopParticleAttachment.Test_AttachmentType = ParticleAttachment.AttachmentType.Static;
        m_TopParticleAttachment.Test_AttachmentParticle = AttachmentParticle.Top;

        m_BottomParticleAttachment.Test_AttachmentType = ParticleAttachment.AttachmentType.Dynamic;
        m_BottomParticleAttachment.Test_AttachmentParticle = AttachmentParticle.Bottom;

        m_Rope.OnStart();

        Assert.That(m_Rope.AttachmentConstraints.Length, Is.EqualTo(1));
        Assert.That(m_Rope.AttachmentConstraints[0].Index, Is.EqualTo(0));

        TestTools.AssertPosition(m_Rope.Particles[0].Position, TopParticlePosition);
        TestTools.AssertPosition(m_Rope.Particles[1].Position, BottomParticlePosition);
    ...
    private Rope CreateRopeWithTwoParticle(Vector3 topParticlePosition, Vector3 bottomParticlePosition, AttachmentParticle cursorAttachmentParticle)
    {
        var rope = new GameObject().AddComponent<Rope>();
        m_TopParticleAttachment = TestTools.CreateParticleAttachment(rope, AttachmentParticle.Top, ParticleAttachment.AttachmentType.Dynamic);
        m_BottomParticleAttachment = TestTools.CreateParticleAttachment(rope, AttachmentParticle.Bottom, ParticleAttachment.AttachmentType.Dynamic);
