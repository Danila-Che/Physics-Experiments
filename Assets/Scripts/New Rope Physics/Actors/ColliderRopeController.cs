...
{
	public class ColliderRopeController : MonoBehaviour, ISecondaryActorController
	{
		[Min(0.01f)]
		...
		[SerializeField] private BoxColliderController m_BoxCollider;
		[SerializeField] private SolfBoxColliderController m_SolfBoxCollider;
		[Min(0)]
		[SerializeField] private int m_ParticleIndexOfBoxCollider;
		...
		public void InitWithSolver(Solver solver)
		{
			...
			{
				m_Rope.CreateRope(solver, m_BoxCollider.GetParticleIndex(m_ParticleIndexOfBoxCollider), m_TopPoint.Id);
			}
			else if (m_SolfBoxCollider != null)
			{
				m_Rope.CreateRope(solver, m_SolfBoxCollider.GetParticleIndex(m_ParticleIndexOfBoxCollider), m_TopPoint.Id);
			}
